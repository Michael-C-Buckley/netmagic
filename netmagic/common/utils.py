    ...
    for intf in intf_list:
        ...
        if match:
            ...
            parsed_interfaces.append(
                (prefix or "", int(dev_stack), int(module), int(slot), intf)
            )
...
def get_vlan_string(vlans: InterfaceVLANs) -> str:
    ...
    trunk_str = (
        sub(str(vlans.native), f"N{vlans.native}", vlans.tags)
        if vlans.native
        else vlans.tags
    )
    trunk_str = (
        sub(str(vlans.dual), f"D{vlans.dual}", trunk_str) if vlans.dual else trunk_str
    )

    return trunk_str


def convert_vlan_string_to_object(
    vlan_str: str, host: str, interface: str
) -> InterfaceVLANs:
    """
    ...
    if len(vlan_str.strip().split(",")) == 1:
        kwargs = {"access": vlan_str, "mode": "access"}
    if special_matches := search(r"(N)(D)(\d+)", vlan_str):
        ...
        kwargs = {"mode": "trunk", "tags": vlan_str}
