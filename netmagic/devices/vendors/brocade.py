...
class BrocadeSwitch(Switch):
    ...
    def get_interface_status(
        ...
        int_status.fsm_output = {
            i["interface"]: InterfaceStatus(host=self.hostname, **i) for i in fsm_data
        }
    ...
    def get_optics(self, template: str | bool = None) -> ResponseGroup:
        ...
        optical_interfaces = [
            i["interface"] for i in media.fsm_output if search(r"(?i)sfp", i["medium"])
        ]

        optics_responses = [
            self.command(f"show optic {intf}") for intf in optical_interfaces
        ]
        ...
        if template is not False:
            ...
            optics_data = [
                optics_response.response for optics_response in optics.responses
            ]
            fsm_data = [self.fsm_parse(i, template) for i in optics_data]
            optics.fsm_output = {
                i["interface"]: InterfaceOptics.create(self.hostname, **i)
                for i in chain(*fsm_data)
            }
    ...
    def get_lldp(self, template: str | bool = None) -> CommandResponse:
        ...
        lldp.fsm_output = {
            i["interface"]: InterfaceLLDP(host=self.hostname, **i) for i in fsm_data
        }
    ...
    def get_tdr_data(
        ...
        """
        input_kwargs = {
            k: v for k, v in locals().items() if k in get_param_names(self.get_tdr_data)
        }
    ...
    def get_interface_vlans(
        ...
        for interface, kwargs in results.items():
            ...
            output[interface] = InterfaceVLANs(
                host=self.hostname, interface=interface, **kwargs
            )
