...
IPV4_PATTERN = rf"(?:{IPV4_OCTET}\.){{3}}{IPV4_OCTET}"
IPV4_RESERVED_RANGE = (
    r"(?:^10\.)|(?:^127\.)|(?:^172\.(?:1[6-9]|2[0-9]|3[01]))|(?:^192\.168\.)"
)
BINARY_OCTET_8BIT = r"(?:0|128|192|224|240|248|252|254|255)"
IPV4_SUBNET_MASK = rf"(:?{BINARY_OCTET_8BIT}\.){{3}}{BINARY_OCTET_8BIT}"
...
MIXED_IPV6 = rf"::(?:[fF]{{4}}:)?{IPV4_PATTERN}"
IPV6_PATTERN = rf"{BASIC_IPV6}|{MIXED_IPV6}"
    ...
    for line in input_string.split("\n"):
        ...
        if swap_match := search(r"\#(\w+)\#", line):
            if swap_pattern := globals().get(swap_match.group(1)):
                ...
                raise ValueError(
                    f"Template swap did not resolve a matching regex pattern: `{swap_match}` in `{template}.textfsm`"
                )
        template_string = f"{template_string}\n{line}" if template_string else line
...
def parser_preparation(template: str, vendor: str):
    ...
    try:
        ...
        if search(r"Value", template) and search(r"Start", template):
            ...
            raise ValueError(
                "`template` must either be a file path, internal template, or template passed directly as a string"
            )
...
def flatten_fsm_output(prime_key: str, fsm_output: FSMOutputT) -> FSMOutputT:
    ...
    for item in fsm_output:
        try:
            ...
            raise KeyError(
                "`prime_key` must be a common element to collect other values"
            )
